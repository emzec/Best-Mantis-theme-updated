...
# along with MantisBT.  If not, see <http://www.gnu.org/licenses/>.
	...
	echo '<br />';
	echo '<table class="width100" cellspacing="0" id="view-issue-details">';
	echo '<tr class="small-text">';

	# Form Title
	echo '<td colspan="', $t_bugslist ? '6' : '6', '" ><div style="float:left">';

	echo $tpl_form_title;

	echo ' ['.$tpl_project_name.']&#160;<span class="small">';
	...
	# prev/next links
	if ( $t_bugslist ) {
		echo '</div><div style="float:right"><span class="thread-nav">';

		$t_bugslist = explode( ',', $t_bugslist );
		$t_index = array_search( $f_bug_id, $t_bugslist );
		if ( false !== $t_index ) {
			if ( isset( $t_bugslist[$t_index-1] ) ) {
				print_bracket_link( 'view.php?id='.$t_bugslist[$t_index-1], '&laquo; previous' );
			}

			if ( isset( $t_bugslist[$t_index+1] ) ) {
				print_bracket_link( 'view.php?id='.$t_bugslist[$t_index+1], 'next &raquo;' );
			}
		}
		echo '</span>';
	}
	...
	if ( $tpl_top_buttons_enabled ) {
		echo '<tr align="center"  class="small-text">';
	...
	if ( $tpl_show_id || $tpl_show_project || $tpl_show_category || $tpl_show_view_state || $tpl_show_date_submitted || $tpl_show_last_updated ) {
		# Labels
		echo '<!--<tr class="small-text-heading">';
		...
		echo '<tr ', helper_alternate_class(), ' class="small-text">';
	...
	if ( $tpl_show_reporter ) {
		echo '<tr><td class="title"></td><td colspan="5"><table id="thread-view-summary" cellspacing="0"  align="left"><tr ', helper_alternate_class(), ' class="summary">';

		$t_spacer = 4;

		# Reporter
		if ( $tpl_show_reporter ) {
			echo '<td class="title">Thread ID</td><td>',$tpl_formatted_bug_id,'</td>';
			echo '<td class="title">Date Submitted</td><td>', $tpl_date_submitted, '</td>';
			echo '</td><td class="title">Last Update</td><td>', $tpl_last_updated, '</td>';
		} else {
			$t_spacer += 2;
		}
		echo '</tr>';
	...
	if ( $tpl_show_handler || $tpl_show_due_date ) {
		...
		# Handler
		if ( $tpl_show_handler ) {
			echo '<td class="title">', lang_get( 'assigned_to' ), '</td>';
			echo '<td>';
			print_user_with_subject( $tpl_bug->handler_id, $tpl_bug_id );
			echo '</td>';
			echo '<td class="title">', lang_get( 'reporter' ), '</td><td>',
			print_user_with_subject( $tpl_bug->reporter_id, $tpl_bug_id ),'</td>';
			echo '</td><td class="title">Category</td><td>', $tpl_category, '</td>';
		} else {
			$t_spacer += 2;
		}
		...
		# Due Date
		if ( $tpl_show_due_date ) {
			echo '<td class="title">', lang_get( 'due_date' ), '</td>';
	...
	if ( $tpl_show_priority || $tpl_show_severity || $tpl_show_reproducibility ) {
		...
		if ( $tpl_show_priority ) {
			echo '<td class="title">', lang_get( 'priority' ), '</td>';
		...
		if ( $tpl_show_severity ) {
			echo '<td class="title">', lang_get( 'severity' ), '</td>';
		...
		if ( $tpl_show_reproducibility ) {
			echo '<td class="title">', lang_get( 'reproducibility' ), '</td>';
	...
	if ( $tpl_show_status || $tpl_show_resolution ) {
		...
		# Status
		if ( $tpl_show_status ) {
			echo '<td class="title">', lang_get( 'status' ), '</td>';
			echo '<td><span class="status-circle" style="background:'.get_status_color( $tpl_bug->status ).'">';
			echo $tpl_status.'</span></td>';
		} else {
			$t_spacer += 2;
		}
		...
		if ( $tpl_show_resolution ) {
			echo '<td class="title">', lang_get( 'resolution' ), '</td>';
			echo '<td>', $tpl_resolution, '</td><td class="title">View Status</td><td>'.$tpl_bug_view_state_enum.'</td>';
		} else {
			$t_spacer += 2;
		}
		echo '</tr>';
	...
	if ( $tpl_show_projection || $tpl_show_eta ) {
		...
		if ( $tpl_show_projection ) {
			# Projection
			echo '<td class="title">', lang_get( 'projection' ), '</td>';
		...
		# ETA
		if ( $tpl_show_eta ) {
			echo '<td class="title">', lang_get( 'eta' ), '</td>';
			echo '<td>', $tpl_eta, '</td>';
		} else {
			$t_spacer += 2;
		}
	...
	if ( $tpl_show_platform || $tpl_show_os || $tpl_show_os_version ) {
		...
		if ( $tpl_show_platform ) {
			echo '<td class="title">', lang_get( 'platform' ), '</td>';
		...
		# Operating System
		if ( $tpl_show_os ) {
			echo '<td class="title">', lang_get( 'os' ), '</td>';
			echo '<td>', $tpl_os, '</td>';
		} else {
			$t_spacer += 2;
		}
		...
		if ( $tpl_show_os_version ) {
			echo '<td class="title">', lang_get( 'os_version' ), '</td>';
	...
	if ( $tpl_show_product_version || $tpl_show_product_build ) {
		...
		# Product Version
		if ( $tpl_show_product_version ) {
			echo '<td class="title">', lang_get( 'product_version' ), '</td>';
			echo '<td>', nl2br($tpl_product_version_string), '</td>';
		} else {
			$t_spacer += 2;
		}
		...
		if ( $tpl_show_product_build ) {
			echo '<td class="title">', lang_get( 'product_build' ), '</td>';
			echo '<td>', nl2br($tpl_product_build), '</td>';
		} else {
			$t_spacer += 2;
		}
	...
	if ( $tpl_show_target_version || $tpl_show_fixed_in_version ) {
		...
		if ( $tpl_show_target_version ) {
			# Target Version
			echo '<td class="title">', lang_get( 'target_version' ), '</td>';
		...
		# fixed in version
		if ( $tpl_show_fixed_in_version ) {
			echo '<td class="title">', lang_get( 'fixed_in_version' ), '</td>';
	...
	if ( $tpl_show_summary ) {
		echo '<tr ', helper_alternate_class(), '>';
		echo '<td class="title">', lang_get( 'summary' ), '</td>';
	...
	if ( $tpl_show_description ) {
		echo '<tr ', helper_alternate_class(), '>';
		echo '<td class="title"><p>', lang_get( 'description' ), '</p></td>';
	...
	if ( $tpl_show_steps_to_reproduce ) {
		echo '<tr ', helper_alternate_class(), '>';
		echo '<td class="title"><p>', lang_get( 'steps_to_reproduce' ), '</p></td>';
	...
	if ( $tpl_show_additional_information ) {
		echo '<tr ', helper_alternate_class(), '>';
		echo '<td class="title"><p>', lang_get( 'additional_information' ), '</p></td>';
	...
	if ( $tpl_show_tags ) {
		echo '<tr ', helper_alternate_class(), '>';
		echo '<td class="title"><p>', lang_get( 'tags' ), '</p></td>';
		echo '<td colspan="5">';
	...
	if ( $tpl_can_attach_tag ) {
		echo '<tr ', helper_alternate_class(), '>';
		echo '<td class="title"><p>', lang_get( 'tag_attach_long' ), '</p></td>';
		echo '<td colspan="5">';
	...
	foreach( $t_related_custom_field_ids as $t_id ) {
		...
		echo '<tr ', helper_alternate_class(), '>';
		echo '<td class="title"><p>', string_display( lang_get_defaulted( $t_def['name'] ) ), '</p></td>';
		echo '<td colspan="5">';
	...
	if ( $tpl_show_attachments ) {
		echo '<tr ', helper_alternate_class(), '>';
		echo '<td class="title"><p><a name="attachments" id="attachments" />', lang_get( 'attached_files' ), '<p></td>';
		echo '<td colspan="5">';
